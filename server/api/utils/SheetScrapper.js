...
const doc = new GoogleSpreadsheet(
  "1CBK_awNR3w_jx9vZz_DefVWQAsBjagsxppyhMqx1VLM"
); //google sheet id
(async () => {
  ...
  console.log(
    `total ${problemList.length - 1} problems scrapped with ${faults} issues`
  );
  // console.log(problemList[10]);
  await pushtoDB();
  console.log(
    "\n\nUpdated Problems to the Database Complete!\n\nPress CTRL + C to exit....."
  );
})();

async function pushtoDB() {
  await Problem.deleteMany({}, function () {
  ...
  for (let i = 1; i < problemList.length; i++) {
    ...
    try {
      await problem.save();
      console.log(
        `updated problems: ${problemList[i].judge} ${problemList[i].problemID}`
      );
    } catch (error) {
      console.log(error);
    }
